...
def get_bills(model=Bill):
    new_month = True  # assuming no bills exist
    if Bill.objects.all():
        # if some bills already exists, we will compare the latest bill date with today
        latest_bill = Bill.objects.filter(due_date__isnull=False).latest("due_date")
        latest_date = latest_bill.due_date
        new_month = (latest_date.month < TODAY.month) and (
            latest_date.year == TODAY.year
        )
    if new_month:
        add_new_bills(model)
    return model
